...
const allComps = {
      ...
      gf: _gf(MATCHES.filter(function (m) { return m.result !== null; })),
      ga: _ga(MATCHES.filter(function (m) { return m.result !== null; }))
};

console.log(allComps.wins); /* ═══ end DATA ═══════════════════════════════════════ */
...
function renderSchedule() {
  document.getElementById("sec-schedule").innerHTML =
    '<h2 class="card-title-gp"><span class="material-symbols-outlined title-ico">calendar_today</span>Weekly Schedule</h2>' +
    '<div class="timeline">' +
    DATA.schedule
      .map(function (item) {
        return (
